...
open_files  = random.sample([f for f in all_files if "_open"  in f.name], 500)
close_files = random.sample([f for f in all_files if "_close" in f.name], 500)

ears, labels = [], []
total = len(open_files) + len(close_files)
for i, (fpath, label) in enumerate([(f,0) for f in open_files]+[(f,1) for f in close_files]):
...
print("-" * 50)
for thr in np.arange(0.220, 0.260, 0.001):
